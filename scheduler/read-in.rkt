...
(define (get-child-names variable network)
  (hash-ref (belief-network-updates-map network) variable null))
  ...
  (filter
   (lambda (belief)
     (andmap
      (lambda (condition)
               (cond
                 [(member variable (hash-ref (belief-network-updates-map network) (condition-pair-course condition) null))
